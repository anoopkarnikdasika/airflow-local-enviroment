...
resources = ['Patient', 'Coverage', 'Condition', 'Observation', 'DiagnosticReport', 'Procedure', 'Immunization', 'Encounter', 'Medication', 'MedicationRequest', 'Claim', 'ExplanationOfBenefit', 'Coverage', 'InsurancePlan', 'Observation']
actualresource = 'ExplanationOfBenefit'
file_type = 'csv'
...
def ingest_to_smilecdr(ds, **kwargs):
    ...
    input_df = pd.read_csv(input_file['Body'],keep_default_na=False,dtype={
        'Patient_Id': str, 'Provided_Number': str, 'Diagnosis Code': str, 'Procedure Code' :str
    })
    ingestion_success = []
    ingestion_failure = []
    ingested_ids = []
    def ingest(i):
        ...
        explanationofbenefit['identifier']= list()
        explanationofbenefit['identifier'].append({'system':'data_source','value':'Scan'})
        if row['Status']!='':
            status = row['Status']
            if status == 'Active':
                status = 'active'
        ...
        if row['Recorded_On']!='':
            explanationofbenefit['created'] = row['Recorded_On']
        if row['Claim_Number']!='':
            identifier = {}
            identifier['value'] = row['Claim_Number']
            identifier['system'] = "http://hl7.org/fhir/us/carin-bb/CodeSystem/C4BBIdentifierType"
            explanationofbenefit['identifier'].append(identifier)
        if row['Claim_Type']!='':
            explanationofbenefit['type'] = {'coding':[{'code':row['Claim_Type'],'display':row['Claim_Type']}]}
        if row['Total_Claim_Amount']!='' or row['Paid_To_Provider']!='':
            explanationofbenefit['total'] = list()
            if row['Total_Claim_Amount']!='':                
                total = {}
                total['amount'] ={'value':row['Total_Claim_Amount'][:-1],'currency':row['Total_Claim_Amount'][-1]}
                total['category'] = {'coding':[{'code':'submitted'}]}
                explanationofbenefit['total'].append(total)
            if row['Paid_To_Provider']!='':
                total = {}
                total['amount'] ={'value':row['Paid_To_Provider'][:-1],'currency':row['Paid_To_Provider'][-1]}
                total['category'] = {'coding':[{'code':'paidtoprovider'}]}
                explanationofbenefit['total'].append(total)
        if row['Start_Date']!='':
            explanationofbenefit['billablePeriod'] = {'start':str(row['Start_Date'])}
            if row['End_Date']!='':
                explanationofbenefit['billablePeriod'] = {'start':str(row['Start_Date']),'end':str(row['End_Date'])}
        if row['Diagnosis']!='':
            explanationofbenefit['diagnosis'] = list()
            diagnosis = {}
            diagnosis['diagnosisCodeableConcept'] = {'text':row['Diagnosis'],'coding':[{'display':row['Diagnosis'],'code':str(row['Diagnosis Code'])}]}
            explanationofbenefit['diagnosis'].append(diagnosis)
        if row['Procedure']!='':
            explanationofbenefit['procedure'] = list()
            procedure = {}
            procedure['procedureCodeableConcept'] = {'text':row['Procedure'],'coding':[{'display':row['Procedure'],'code':str(row['Procedure Code'])}]}
            explanationofbenefit['procedure'].append(procedure)
        ...
        explanationofbenefit['patient'] = {'reference':'Patient/'+patient_response['entry'][0]['resource']['id']}
        practitioner = {}
        practitioner['resourceType']= 'Practitioner'
        if row['Provider']!='':
            practitioner['name'] = list()
            practitioner['name'].append({'text':row['Provider']})
        if row['Provider_Number']!='':
            practitioner['telecom'] = list()
            practitioner['telecom'].append({'system':'phone','value':str(row['Provider_Number'])})
        print(json.loads(json.dumps(practitioner)))
        practitioner_response = requests.post(target_ingestion_endpoint+tenant_name+'/Practitioner',
                json=json.loads(json.dumps(practitioner)),
                auth=(target_ingestion_username , target_ingestion_password)).json()
        print(practitioner_response)
        explanationofbenefit['provider'] = {'reference': 'Practitioner/'+practitioner_response['id']}
        print(json.loads(json.dumps(explanationofbenefit)))
        response = requests.post(target_ingestion_endpoint+tenant_name+'/'+actualresource,
                json=json.loads(json.dumps(explanationofbenefit)),
                auth=(target_ingestion_username , target_ingestion_password)).json()
        print(response)
        print(response['id'])
