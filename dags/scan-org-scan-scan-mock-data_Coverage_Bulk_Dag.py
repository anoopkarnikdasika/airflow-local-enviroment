...
def ingest_to_smilecdr(ds, **kwargs):
    ...
    def ingest(i):
        ...
        coverage.identifier = list()
        identifier2 = Identifier.construct()
        identifier2.system = "data_source"
        identifier2.value = "Scan"
        coverage.identifier.append(identifier2)
        if row['Coverage_Type']!='':
            codeableConcept = CodeableConcept.construct()
            codeableConcept.text = row['Coverage_Type']
            codeableConcept.coding = list()
            codeableConcept.coding.append({'display':row['Coverage_Type'],'code':row['Coverage_Type']})
            coverage.type = codeableConcept
        if row['Subscriber_ID']!='':
            coverage.subscriberId = row['Subscriber_ID']
        if row['Relationship']!='':
            codeableConcept = CodeableConcept.construct()
            codeableConcept.text = row['Relationship']
            codeableConcept.coding = list()
            codeableConcept.coding.append({'display':row['Relationship'],'code':row['Relationship']})
            coverage.relationship = codeableConcept
        if row['Issuer_Organization']!='':
            payor = {'display':row['Issuer_Organization']}
            coverage.payor = list()
            coverage.payor.append(payor)
        if row['Policy_Number']!='':
            identifier = Identifier.construct()
            identifier.value = row['Policy_Number']
            identifier.system = 'Policy-Number'        
            coverage.identifier.append(identifier)
        if row['plan_id']!='':
            identifier = Identifier.construct()
            identifier.value = row['plan_id']
            identifier.system = 'Plan-Id'
            coverage.identifier.append(identifier)
        coverage = coverage.dict()
        if row['Start_Date']!='':
            coverage['period'] = {'start':str(row['Start_Date'])}
            if row['End_Date']!='':
                coverage['period'] = {'start':str(row['Start_Date']),'end':str(row['End_Date'])}
        if row['Plan_Name']!='' or row['Plan_Type']!='':
            coverage['class'] = list()
            cla = {}
            if row['Plan_Name']!='': 
                cla['name'] = row['Plan_Name']
            if row['Plan_Type']!='':
                cla['type'] = {'text':row['Plan_Type'],'coding':[{'display':row['Plan_Type'],'code':row['Plan_Type']}]}
            coverage['class'].append(cla)
        if row['Services_in_Coverage']!='' or row['Total_Amount']!='':
            coverage['costToBeneficiary'] = list()
            costToBeneficiary = {}
            if row['Services_in_Coverage']!='':
                value = row['Services_in_Coverage']
                costToBeneficiary['type'] = {'text':value,'coding':[{'display':value,'code':value}]}
            if row['Total_Amount']!='':
                costToBeneficiary['valueQuantity'] = {'value':str(row['Total_Amount'])}
            coverage['costToBeneficiary'].append(costToBeneficiary)
        ...
        coverage['beneficiary']= {'reference':'Patient/'+patient_response['entry'][0]['resource']['id']}
        coverage['subscriber']= {'reference':'Patient/'+patient_response['entry'][0]['resource']['id']}
        coverage['policyHolder']= {'reference':'Patient/'+patient_response['entry'][0]['resource']['id']}
        print(json.loads(json.dumps(coverage)))
        response = requests.post(target_ingestion_endpoint+tenant_name+'/'+actualresource,
                json=json.loads(json.dumps(coverage)),
                auth=(target_ingestion_username , target_ingestion_password)).json()
        print(response)
        print(response['id'])
