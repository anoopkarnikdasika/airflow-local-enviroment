...
def ingest_to_smilecdr(ds, **kwargs):
    ...
    def ingest(i):
        ...
        careTeam['identifier'] = list()
        careTeam['identifier'].append({'system':'data_source','value':'Scan'})
        if row['Care Team Name']!='':
            careTeam['name'] = row['Care Team Name']
        if row['Category']!='':
            careTeam['category'] = list()
            careTeam['category'].append({'text':row['Category'],'coding':[{'code':row['Category'],'display':row['Category']}]})
        if row['Status']!='':
            careTeam['status'] = row['Status']
        careTeam['period'] = {}
        if row['Start date']!='':
            careTeam['period']['start'] = row['Start date']
        if row['End date']!='':
            careTeam['period']['end'] = row['End date']
        if row['Note']!='':
            careTeam['note'] = list()
            annotation = {}
            annotation['text'] = row['Note']
            careTeam['note'].append(annotation)
        careTeam['participant'] = []
        careTeam['participant'].append({})
        if row['Org. Name']!='':
            careTeam['participant'][0]['onBehalfOf'] = {'display':row['Org. Name']}
        if row['Name']!='':
            careTeam['participant'][0]['member'] = {'display':row['Name']}
        if row['Role']!='':
            careTeam['participant'][0]['role'] = []
            careTeam['participant'][0]['role'].append({'text':row['Role'],'coding':[{'code':row['Role'],'display':row['Role']}]})
        if row['Reason']!='':
            careTeam['reasonCode'] = list()
            careTeam['reasonCode'].append({'text':row['Reason'],'coding':[{'code':row['Reason'],'display':row['Reason']}]})
        if row['Telephone ']!='':
            careTeam['telecom'] = list()
            careTeam['telecom'].append({'value':row['Telephone ']})
        print(json.loads(json.dumps(careTeam)))
        ...
        if row['Associated careplan name']!='':
            carePlan['title'] = row['Associated careplan name']
        if row['Associated careplan activites']!='':
            carePlan['activity'] = list()
            carePlan['activity'].append({'detail':{'code':{'text':row['Associated careplan activites'],'coding':[{'display':row['Associated careplan activites']}]}}})
