...
def ingest_to_smilecdr(ds, **kwargs):
    ...
    input_df = pd.read_csv(input_file['Body'],keep_default_na=False,dtype={
        'NDC_Code':str, 'Patient_Id': str, 'Vaccine_Dose_No':str
    })
    ingestion_success = []
    ingestion_failure = []
    ingested_ids = []
    def ingest(i):
        ...
        if row['Status']!='':
            immunization['status']= row['Status']
        immunization['identifier']= list()
        immunization['identifier'].append({'system':'data_source','value':'Scan'})
        if row['Immunization_Name']!='':
            codeableConcept = {}
            codeableConcept['text'] = row['Immunization_Name']
            codeableConcept['coding'] = list()
            codeableConcept['coding'].append({'display':row['Immunization_Name'],'code':row['NDC_Code']})
            immunization['vaccineCode'] = codeableConcept
        if row['Administered_On']!='':
            immunization['occurrenceDateTime'] = row['Administered_On']
        if row['Vaccine_Manufacturer']!='':    
            immunization['manufacturer'] = {'display':row['Vaccine_Manufacturer']}
        if row['Vaccine_Lot_No']!='':
            immunization['lotNumber'] = row['Vaccine_Lot_No']
        if row['Body_Site'] !='':
            codeableConcept = {}
            codeableConcept['coding'] = list()
            codeableConcept['coding'].append({'display':row['Body_Site'],'code':row['Body_Site']})
            codeableConcept['text'] = row['Body_Site']
            immunization['site'] = codeableConcept
        if row['Route'] != '':
            codeableConcept = {}
            codeableConcept['coding'] = list()
            codeableConcept['coding'].append({'display':row['Route'],'code':row['Route']})
            codeableConcept['text'] = row['Route']
            immunization['route'] = codeableConcept
        if row['Dosage'] != '':
            dosage_value = row['Dosage'].split(' ')[0]
            dosage_currency = row['Dosage'].split(' ')[1]
            immunization['doseQuantity'] = {'value':dosage_value,'unit':dosage_currency}
        if row['Recorded_By'] != '':
            immunization['performer'] = list()
            function = {'coding':[{
                'system':'http://terminology.hl7.org/CodeSystem/v2-0443',
                'code':'AP',
                'display':'Administering Provider'
            }]}
            actor = {'display':row['Recorded_By']}
            immunization['performer'].append({'actor':actor,'function':function})
        if row['Reason']!='':
            immunization['reasonCode'] = list()
            codeableConcept = {}
            codeableConcept['coding'] = list()
            codeableConcept['coding'].append({'display':row['Reason'],'code':row['Reason']})
            codeableConcept['text'] = row['Reason']
            immunization['reasonCode'].append(codeableConcept)
        if row['Reaction'] != '':
            immunization['reaction'] = list()
            detail = {'display':row['Reaction']}
            immunization['reaction'].append({'detail':detail})
        if row['Vaccine_Dose_No'] !='':
            immunization['protocolApplied'] = list()
            immunization['protocolApplied'].append({'doseNumberPositiveInt':row['Vaccine_Dose_No']})
        if row['Vaccine_expiration_date']!='':
            immunization['expirationDate'] = row['Vaccine_expiration_date']
        if row['Notes']!='':
            immunization['note'] = list()
            annotation = {}
            annotation['text'] = row['Notes']
            immunization['note'].append(annotation)
        location = {}
        ...
        immunization['location'] = {'reference': 'Location/'+location_response['id']}
        ...
        immunization['patient'] = {'reference':'Patient/'+patient_response['entry'][0]['resource']['id']}
        print(json.dumps(immunization))
        response = requests.post(target_ingestion_endpoint+tenant_name+'/'+actualresource,
                json=json.loads(json.dumps(immunization)),
                auth=(target_ingestion_username , target_ingestion_password)).json()
        print(response)
        print(response['id'])
