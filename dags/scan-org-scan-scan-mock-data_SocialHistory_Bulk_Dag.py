...
# The DAG definition
dag = DAG(
    dag_id=name+'_SocialHistory_Bulk_Dag',
...
def ingest_to_smilecdr(ds, **kwargs):
    ...
    def ingest(i):
        ...
        category.coding.append({'display':'social-history','code':'social-history'})
        ...
        if row['Onset Time']!='':
            observation.effectiveDateTime = row['Onset Time']
        if row['Recorded On']!='':
            observation.issued = row['Recorded On']+"T00:00:00Z"
        if row['Frequency']!='':
            valueCodeableConcept = CodeableConcept.construct()
            valueCodeableConcept.coding = list()
            valueCodeableConcept.coding.append({'display':row['Frequency']})
            valueCodeableConcept.text = row['Frequency']
        ...
        if row['Recorded By']!='':
            ...
            practitioner['name'].append({'text':row['Recorded By']})
        ...
        observation.performer = [{'reference': 'Practitioner/'+practitioner_response['id']}]
