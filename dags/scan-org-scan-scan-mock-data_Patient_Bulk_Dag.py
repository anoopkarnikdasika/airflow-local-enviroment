...
def ingest_to_smilecdr(ds, **kwargs):
    ...
    input_df = pd.read_csv(input_file['Body'],keep_default_na=False,dtype={
        'Phone number':str, 'Zipcode':str, 'Patient ID': str
    })
    ingestion_success = []
    ingestion_failure = []
    ingested_ids = []
    def ingest(i):
        ...
        identifier1.system = "Scan Patient ID"
        identifier1.value = row['Patient ID']
        ...
        name.given = list()
        name.given.append(row['First name'])
        name.family = row['Last name']
        ...
        year = row['DOB (MM/DD/YYYY)'][-4:]
        month = row['DOB (MM/DD/YYYY)'][:2]
        date = row['DOB (MM/DD/YYYY)'][3:5]
        patient.birthDate = year + '-' + month + '-' + date
        if row['Gender'] == 'Male':
            patient.gender = 'male'
        ...
        address.line = list()
        address.line.append(row['Address'])
        ...
        cursor = conn.cursor()
        cursor.execute(
            f"insert into {mpowered_member_details_table} (affiliate_id,first_name,last_name,email_id,phone,fhir_reference_id,birth_date,created_at,updated_at) values({affiliate_id},'{first_name}','{last_name}','{email_id}','{phone}','{patient_id}','{birth_date}','{time}','{time}') ")
        conn.commit()
        cursor.close()
        cursor = conn.cursor()
        cursor.execute(
            f"insert into {mpowered_member_table} (patient_id,first_name,last_name,email,phone,birth_date,org_id,affiliate_id,lob_id,created_on,updated_on) values({patient_id},'{first_name}','{last_name}','{email_id}',{phone},'{birth_date}',{org_id},{affiliate_id},{lob_id},'{time}','{time}') ")
        conn.commit()
        cursor.close()
