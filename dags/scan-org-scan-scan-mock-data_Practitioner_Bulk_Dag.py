...
def ingest_to_smilecdr(ds, **kwargs):
    ...
    def ingest(i):
        ...
        if row['active']!='':
            practitioner.active = row['active']
        if row['gender']!='':
            practitioner.gender = row['gender']
        if row['Qualification']!='':
            practitioner.qualification = list()
            practitioner.qualification.append({'code':{'text':row['Qualification']}})
        if row['language']!='':
            practitioner.communication = list()
            practitioner.communication.append({'text':row['language']})
        print(json.loads(practitioner.json()))
        response = requests.post(target_ingestion_endpoint+tenant_name+'/'+actualresource,
                json=json.loads(practitioner.json()),
