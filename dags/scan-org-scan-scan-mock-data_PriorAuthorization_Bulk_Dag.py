...
actualresource = 'PriorAuthorization'
file_type = 'csv'
...
def ingest_to_smilecdr(ds, **kwargs):
    ...
    def ingest(i):
        ...
        serviceRequest['identifier'] = list()
        serviceRequest['identifier'].append({'system':'data_source','value':'Scan'})
        if row['Status']!='':
            serviceRequest['status'] = row['Status'].lower().strip()
        if row['Priority']!='':
            serviceRequest['priority'] = row['Priority'].lower().strip()
        if row['Request Date']!='':
            serviceRequest['authoredOn'] = row['Request Date']
        if row['Due Date']!='':
            serviceRequest['occurrenceDateTime'] = row['Due Date']
        if row['Service Location']!='':
            serviceRequest['locationReference'] = list()
            serviceRequest['locationReference'].append({'display':row['Service Location']})
        serviceRequest['occurrencePeriod'] = {}
        if row['Service Start Date']!='':
            serviceRequest['occurrencePeriod']['start'] = row['Service Start Date']
        if row['Service End Date']!='':
            serviceRequest['occurrencePeriod']['end'] = row['Service End Date']
        serviceRequest['reasonCode'] = list()
        serviceRequest['reasonCode'].append({})
        if row['Diagnosis Name']!='':
            serviceRequest['reasonCode'][0]['text'] = row['Diagnosis Name']
            serviceRequest['reasonCode'][0]['coding'] = list()
            serviceRequest['reasonCode'][0]['coding'].append({})
            serviceRequest['reasonCode'][0]['coding'][0]['display'] = row['Diagnosis Name']
            if row['Diagnosis Code']!='':
                serviceRequest['reasonCode'][0]['coding'][0]['code'] = row['Diagnosis Code']
        if row['Note']!='':
            serviceRequest['note'] = list()
            annotation = {}
            annotation['text'] = row['Note']
            serviceRequest['note'].append(annotation)
        if row['Units']!='':
            serviceRequest['quantityQuantity'] = {'value':str(row['Units'])}
        if row['Category ']!='':
            serviceRequest['category'] = list()
            serviceRequest['category'].append({'text':row['Category '],'coding':[{'display':row['Category '],'code':row['Category ']}]})
        response = requests.post(target_ingestion_endpoint+tenant_name+'/ServiceRequest',
                json=json.loads(json.dumps(serviceRequest)),
