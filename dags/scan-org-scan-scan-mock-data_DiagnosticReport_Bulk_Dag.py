...
actualresource = 'DiagnosticReport'
file_type = 'csv'
...
def ingest_to_smilecdr(ds, **kwargs):
    ...
    def ingest(i):
        ...
        if row['Status']!='':
            diagnosticreport.status = row['Status']
        diagnosticreport.identifier = list()
        identifier2 = Identifier.construct()
        identifier2.system = "data_source"
        identifier2.value = "Scan"
        diagnosticreport.identifier.append(identifier2)
        if row['Category_Code']!='':
            diagnosticreport.category = list()
            codeableConcept = CodeableConcept.construct()
            codeableConcept.coding = list()
            codeableConcept.coding.append({'display':row['Category_Code'].lower(),'code':row['Category_Code'].lower()})
            codeableConcept.text = row['Category_Code'].lower()
            diagnosticreport.category.append(codeableConcept)
        if row['Loinc_Code']!='':
            codeableConcept = CodeableConcept.construct()
            codeableConcept.text = row['Name']
            codeableConcept.coding = list()
            codeableConcept.coding.append({'display':row['Name'],'code':row['Loinc_Code']})
            diagnosticreport.code = codeableConcept
        if row['Facility']!='':
            diagnosticreport.encounter = {'display':row['Facility']}
        if row['Recorded_On']!='':
            diagnosticreport.effectiveDateTime = row['Recorded_On']
        if row['Issued_Date']!='':
            diagnosticreport.issued = row['Issued_Date']+'T00:00:00Z'
        if row['Recorded_By']!='':    
            diagnosticreport.performer = list()
            diagnosticreport.performer.append({'display':row['Recorded_By']})
        if row['Issuer_of_the_Report']!='':
            diagnosticreport.resultsInterpreter = list()
            diagnosticreport.resultsInterpreter.append({'display':row['Issuer_of_the_Report']})
        if row['Value']!='':
            diagnosticreport.result = list()
            diagnosticreport.result.append({'display':row['Value']})
        if row['Note']!='':
            diagnosticreport.conclusion = row['Note']
        else:
            diagnosticreport.conclusion = "NA"
        ...
        diagnosticreport.subject= {'reference':'Patient/'+patient_response['entry'][0]['resource']['id']}
        observation = {}
        observation['resourceType']= 'Observation'
        if row['Value']!='':
            observation['valueQuantity'] = {'value':str(row['Value']),'unit':row['unit']}
        if row['Lab_Result']!='':
            observation['interpretation'] = list()
            observation['interpretation'].append({'text':row['Lab_Result'],'coding':[{'code':row['Lab_Result'].lower(),'display':row['Lab_Result']}]})
        if row['Reference_range']!='':
            values = row['Reference_range'].split(' ')[0]
            unit = row['Reference_range'].split(' ')[1]
            low_value = values.split('-')[0]
            high_value = values.split('-')[1]
            observation['referenceRange']=list()
            low = {'value':low_value,'unit':unit}
            high = {'value':high_value,'unit':unit}
            observation['referenceRange'].append({'low':low,'high':high})
        print(json.loads(json.dumps(observation)))
        observation_response = requests.post(target_ingestion_endpoint+tenant_name+'/Observation',
                json=json.loads(json.dumps(observation)),
                auth=(target_ingestion_username , target_ingestion_password)).json()
        print(observation_response)
        diagnosticreport.result = list()
        diagnosticreport.result.append({'reference':'Observation/'+observation_response['id']})
        print(json.loads(diagnosticreport.json()))
        response = requests.post(target_ingestion_endpoint+tenant_name+'/'+actualresource,
                json=json.loads(diagnosticreport.json()),
                auth=(target_ingestion_username , target_ingestion_password)).json()
        print(response)
        print(response['id'])
