...
resources = ['PractitionerRole', 'AllergyIntolerance', 'Condition', 'Observation', 'DiagnosticReport', 'Procedure', 'Immunization', 'Encounter', 'Medication', 'MedicationRequest', 'Claim', 'ExplanationOfBenefit', 'Coverage', 'InsurancePlan', 'Observation']
actualresource = 'PractitionerRole'
file_type = 'csv'
...
def ingest_to_smilecdr(ds, **kwargs):
    ...
    def ingest(i):
        ...
        practitionerRole['identifier'] = list()
        identifier1 = {}
        identifier1['system'] = "http://hl7.org/fhir/sid/us-npi"
        identifier1['value'] = str(int(float(row['npi'])))
        practitionerRole['identifier'].append(identifier1)
        identifier2 = {}
        identifier2['system'] = "Plan-Id"
        identifier2['value'] = row['plan id']
        practitionerRole['identifier'].append(identifier2)
        identifier3 = {}
        identifier3['system'] = "data_source"
        identifier3['value'] = "Scan"
        practitionerRole['identifier'].append(identifier3)
        if row['active']!='':
            if str(row['active']) == 'True':
                practitionerRole['active'] = 'true'
            else:
                practitionerRole['active'] = 'false'
        if row['phone']!='':
            practitionerRole['telecom'] = list()
            practitionerRole['telecom'].append({'system':'phone','value':row['phone']})
        if row['address_line']!='':
            practitionerRole['location'] = list()
            address_text = row['address_line'] + ' ' + row['address_city'] + ' ' + row['address_state'] + ' ' + row['address_postal_code'] + ' ' + row['address_country']
            practitionerRole['location'].append({'display':address_text })
        if row['specialty_code']!='':
            practitionerRole['specialty'] = list()
            practitionerRole['specialty'].append({'text':row['specialty_display'],'coding':[{'code':row['specialty_code'],'display':row['specialty_display']}]})
        if row['organzation_name/facility']!='':
            practitionerRole['organization'] = {'display': row['organzation_name/facility']}
        print(f"practitioner url = {target_ingestion_endpoint}{tenant_name}/Practitioner?identifier={str(int(float(row['npi'])))}")
        practitioner_response = requests.get(target_ingestion_endpoint+tenant_name+'/Practitioner?identifier='+str(int(float(row['npi']))),
                auth=(target_ingestion_username , target_ingestion_password)).json()
        print(practitioner_response)
        practitionerRole['practitioner'] = {'reference':'Practitioner/'+practitioner_response['entry'][0]['resource']['id']}
        print(json.loads(json.dumps(practitionerRole)))
        response = requests.post(target_ingestion_endpoint+tenant_name+'/'+actualresource,
                json=json.loads(json.dumps(practitionerRole)),
                auth=(target_ingestion_username , target_ingestion_password)).json()
        print(response)
        print(response['id'])
...
#     cursor = conn.cursor()
#     cursor.execute(
#         "update "+mpowered_resource_table+" set status = 'ingested',last_run='"+time+"' where organization_id = '"+organization_id+"' and affliate_id = '" + affliate_id + "' and lob_id = '" + lob_id+"' and resource='PractitionerRole'")
#     conn.commit()
#     cursor.close()
