...
def ingest_to_smilecdr(ds, **kwargs):
    ...
    def ingest(i):
        ...
        if row['Status']!='':
            encounter.status = row['Status']
        encounter.identifier = list()
        identifier2 = Identifier.construct()
        identifier2.system = "data_source"
        identifier2.value = "Scan"
        encounter.identifier.append(identifier2)
        if row['Encounter_Type']!='':
            encounter.type = list()
            codeableConcept = CodeableConcept.construct()
            codeableConcept.text = row['Encounter_Type']
            codeableConcept.coding = list()
            codeableConcept.coding.append({'system':'http://terminology.hl7.org/CodeSystem/encounter-type','display':row['Encounter_Type'],'code':row['Encounter_Type']})
            encounter.type.append(codeableConcept)
        if row['Service_Type']!='':
            codeableConcept = CodeableConcept.construct()
            codeableConcept.text = row['Service_Type']
            codeableConcept.coding = list()
            codeableConcept.coding.append({'display':row['Service_Type'],'code':row['Service_Type']})
            encounter.serviceType = codeableConcept
        if row['Facility_Name']!='':
            encounter.serviceProvider = {'display':row['Facility_Name']}
        if row['Description']!='':
            encounter.reasonCode = list()
            codeableConcept = CodeableConcept.construct()
            codeableConcept.text = row['Description']
            codeableConcept.coding = list()
            codeableConcept.coding.append({'display':row['Description'],'code':row['Description']})
            encounter.reasonCode.append(codeableConcept)
        if row['Location']!='':
            encounter.location = list()
            location = {'display':row['Location']}
            encounter.location.append({'location':location})
        encounter = encounter.dict()
        if row['Visit_Class']!='':
            encounter['class'] = {'display':row['Visit_Class'],'code':row['Visit_Class']}
        if row['Start_date']!='':
            encounter['period'] = {'start':str(row['Start_date'])}
            if row['End_Date']!='':
                encounter['period'] = {'start':str(row['Start_date']),'end':str(row['End_Date'])}
        practitionerrole = {}
        practitionerrole['resourceType']= 'PractitionerRole'
        if row['Provider_Speciality']!='':
            practitionerrole['specialty'] = list()
            practitionerrole['specialty'].append({'text':row['Provider_Speciality'],'coding':[{'display':row['Provider_Speciality']}]})
        print(json.loads(json.dumps(practitionerrole)))
        practitionerrole_response = requests.post(target_ingestion_endpoint+tenant_name+'/PractitionerRole',
                json=json.loads(json.dumps(practitionerrole)),
                auth=(target_ingestion_username , target_ingestion_password)).json()
        print(practitionerrole_response)
        encounter['participant'] = list()
        individual = {'reference':'PractitionerRole/'+str(practitionerrole_response['id']),'display':row['Provider']}
        encounter['participant'].append({'individual':individual})
        ...
        encounter['subject']= {'reference':'Patient/'+patient_response['entry'][0]['resource']['id']}
        print(json.loads(json.dumps(encounter)))
        response = requests.post(target_ingestion_endpoint+tenant_name+'/'+actualresource,
                json=json.loads(json.dumps(encounter)),
                auth=(target_ingestion_username , target_ingestion_password)).json()
        print(response)
        print(response['id'])
