...
location_add_url = 'http://mph-find-care.mpowered-health.local:8080/find-care/unauth/addlocations'
...
# The DAG definition
dag = DAG(
    dag_id=name+'_'+actualresource+'_Bulk_Dag1',
    schedule_interval = None,
    catchup = False,
...
def standardization_mapped(ds,**kwargs):
    ...
    print(len(input_df))
    input_df = input_df.drop_duplicates(subset=['address_line'])
    print(len(input_df))
    csv_buffer = StringIO()
    input_df.to_csv(csv_buffer, index=False)
...
def ingest_to_smilecdr(ds,**kwargs):
    ...
    def ingest(i):
        ...
        try:
            location_response1 = requests.get(target_ingestion_endpoint+tenant_name+'/'+actualresource+'?address='+address_search,
                                auth=(target_ingestion_username , target_ingestion_password))
        except:
            print(actualresource+'?address='+address_search)
        location_result1 = location_response1.json()
        if location_result1['total']>0:
            try:
                location_response2 = requests.get(target_ingestion_endpoint+tenant_name+'/'+actualresource+'?identifier=mmo&address='+address_search,
                                auth=(target_ingestion_username , target_ingestion_password))
            except:
                print(actualresource+'?identifier=mmo&address='+address_search)
            location_result2 = location_response2.json()
            if location_result2['total']==0:
                ...
                try:
                    ...
                    print(resource['id'])
            elif location_result2['total']>1:
                for i in range(1,location_result2['total']):
                    id = location_result2['entry'][i]['resource']['id']
                    try:
            ...
            contactpoint = ContactPoint.construct()
            ...
            location.identifier.append(identifier1)
            location.identifier.append(identifier2)
            address.line = list()
            address.line.append(row['address_line'])
            address.city = row['address_city']
            address.state = row['address_state']
            address.postalCode = str(row['address_postalcode'])
            location.address = address
            location.telecom = list()
            location.status = "active"
            contactpoint.system = row['telecom_system']
            contactpoint.value = row['telecom_value']
            location.telecom.append(contactpoint)
            try:
                response = requests.post(target_ingestion_endpoint+tenant_name+'/'+actualresource,
                        json=location.dict(),
                        auth=(target_ingestion_username , target_ingestion_password)).json()
                id = response['id']
                # print("Ingested ={}".format(row['address']))
            except:
                print("not able to ingest ={}".format(row['address']))
            data = {}
            identifier = "/Location/"+id
            additional_info = response['address']['line'][0]
            data['identifier'] = identifier
            data['additionalInfo'] = additional_info
            try:
                response = requests.post(location_add_url,json=data)
                code = response['code']
            except:
                ingested_locations.append(identifier)
        if i%1000==0:
            print("rows processed = {}".format(i))
    with concurrent.futures.ThreadPoolExecutor(max_workers=50) as executor:
        res = [executor.submit(ingest,i) for i in range(len(input_df))]
        concurrent.futures.wait(res)
        print("No of new Locations ingested = {}".format(len(ingested_locations)))
    # for j in range(10):
    #     ingest(j)
