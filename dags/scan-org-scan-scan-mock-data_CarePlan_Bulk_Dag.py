...
resources = ['Practitioner', 'AllergyIntolerance', 'Condition', 'Observation', 'DiagnosticReport', 'Procedure', 'Immunization', 'Encounter', 'Medication', 'MedicationRequest', 'Claim', 'ExplanationOfBenefit', 'Coverage', 'InsurancePlan', 'Observation']
actualresource = 'CarePlan'
file_type = 'csv'
...
def ingest_to_smilecdr(ds, **kwargs):
    ...
    def ingest(i):
        ...
        carePlan['identifier'] = list()
        carePlan['identifier'].append({'system':'data_source','value':'Scan'})
        if row['Status']!='':
            carePlan['status'] = row['Status']
        if row['intent']!='':
            carePlan['intent'] = row['intent']
        if row['Name']!='':
            carePlan['title'] = row['Name']
        if row['Category']!='':
            carePlan['category'] = list()
            carePlan['category'].append({'text':row['Category'],'coding':[{'system':'https://www.hl7.org/fhir/valueset-care-plan-category.html','code':row['Category'],'display':row['Category']}]})
        if row['Description']!='':
            carePlan['Description']=row['Description']
        # carePlan['period'] = {}
        # if row['Period_Start']!='':
        #     carePlan['period']['start'] = row['Period_Start']
        # if row['Period_End']!='':
        #     carePlan['period']['end'] = row['Period_End']
        # if row['Recorded On']!='':
        #     carePlan['created'] = row['Recorded On']
        if row['Recorded By']!='':
            carePlan['author'] = {'display':row['Recorded By']}
        if row['Facility']!='':
            carePlan['contributor'] = list()
            carePlan['contributor'].append({'display':row['Facility']})
        if row['Note']!='':
            carePlan['note'] = list()
            annotation = {}
            annotation['text'] = row['Note']
            carePlan['note'].append(annotation)
        if row['Interventions/ Activities']!='':
            carePlan['activity'] = list()
            carePlan['activity'].append({'detail':{'code':{'display':row['Interventions/ Activities'],'coding':[{'code':row['Interventions/ Activities'],'display':row['Interventions/ Activities']}]}}})
        careTeam = {}
        careTeam['resourceType'] = 'CareTeam'
        if row['Care_Team_Name']!='':
            careTeam['name'] = row['Care_Team_Name']
        if row['Category']!='':
            careTeam['category'] = list()
            careTeam['category'].append({'text':row['Category'],'coding':[{'display':row['Category'],'code':row['Category']}]})
        response = requests.post(target_ingestion_endpoint+tenant_name+'/CareTeam',
                json=json.loads(json.dumps(careTeam)),
                auth=(target_ingestion_username , target_ingestion_password)).json()
        print(response)
        print(response['id'])
        carePlan['careTeam'] = list()
        carePlan['careTeam'].append({'reference':'CareTeam/'+str(response['id'])})
        goal = {}
        goal['resourceType'] = 'Goal'
        if row['Goal Status']!='':
            goal['lifecycleStatus'] = row['Goal Status']
        if row['Goal']!='':
            goal['description']= {'text':row['Goal'],'coding':[{'display':row['Goal'],'code':row['Goal']}]}
        response = requests.post(target_ingestion_endpoint+tenant_name+'/Goal',
                json=json.loads(json.dumps(goal)),
                auth=(target_ingestion_username , target_ingestion_password)).json()
        print(response)
        print(response['id'])
        carePlan['goal'] = list()
        carePlan['goal'].append({'reference':'Goal/'+str(response['id'])})
        condition = {}
        condition['resourceType'] = 'Condition'
        if row['Problem Status']!='':
            condition['clinicalStatus'] = {'text':row['Problem Status'],'coding':[{'code':row['Problem Status'],'display':row['Problem Status']}]}
        if row['Problem']!='':
            condition['code']= {'text':row['Problem'],'coding':[{'display':row['Problem'],'code':row['Problem']}]}
        response = requests.post(target_ingestion_endpoint+tenant_name+'/Condition',
                json=json.loads(json.dumps(condition)),
                auth=(target_ingestion_username , target_ingestion_password)).json()
        print(response)
        print(response['id'])
        carePlan['addresses'] = list()
        carePlan['addresses'].append({'reference':'Condition/'+str(response['id'])})
        print(json.loads(json.dumps(carePlan)))
        response = requests.post(target_ingestion_endpoint+tenant_name+'/CarePlan',
                json=json.loads(json.dumps(carePlan)),
                auth=(target_ingestion_username , target_ingestion_password)).json()
        print(response)
        print(response['id'])
...
#     cursor = conn.cursor()
#     cursor.execute(
#         "update "+mpowered_resource_table+" set status = 'ingested',last_run='"+time+"' where organization_id = '"+organization_id+"' and affliate_id = '" + affliate_id + "' and lob_id = '" + lob_id+"' and resource='Practitioner'")
#     conn.commit()
#     cursor.close()
