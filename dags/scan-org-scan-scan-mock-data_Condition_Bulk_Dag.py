...
def ingest_to_smilecdr(ds, **kwargs):
    ...
    def ingest(i):
        ...
        condition.identifier = list()
        identifier2 = Identifier.construct()
        identifier2.system = "data_source"
        identifier2.value = "Scan"
        condition.identifier.append(identifier2)
        if row['Clinicalstatus'] !='':
            clinicalStatus = CodeableConcept.construct()
            clinicalStatus.text = row['Clinicalstatus']
            clinicalStatus.coding = list()
            clinicalStatus.coding.append({'code':row['Clinicalstatus']})
        ...
        if row['Category']!='':
            ...
            condition.category.append(category)
        if row['Severity']!='':
            severity = CodeableConcept.construct()
            severity.coding = list()
            severity.coding.append({'code':row['Severity'],'display':row['Severity']})
            severity.text = row['Severity']
            condition.severity = severity
        if row['Code']!='':
            code = CodeableConcept.construct()
            code.coding = list()
            code.coding.append({'display':row['Condition_Name'],'code':row['Code']})
            code.text = row['Condition_Name']
            condition.code = code
        if row['Body_Site'] !='':
            condition.bodySite = list()
            bodySite = CodeableConcept.construct()
            bodySite.coding = list()
            bodySite.coding.append({'display':row['Body_Site']})
            bodySite.text = row['Body_Site']
            condition.bodySite.append(bodySite)
        if row['Facility'] !='':    
            condition.encounter = {'display':row['Facility']}
        if row['Start_time']!='':
        ...
        if row['Recorded_On'] !='':
            condition.recordedDate = row['Recorded_On']
        if row['Recorded_By'] !='':    
            condition.recorder = {'display':row['Recorded_By']}
            condition.asserter = {'display':row['Recorded_By']}
        if row['Summary']!='':
            condition.stage = list()
            summary = CodeableConcept.construct()
            summary.coding = list()
            summary.coding.append({'display':row['Summary'],'code':row['Summary']})
            summary.text = row['Summary']
            condition.stage.append({'summary':summary})
        if row['Note']!='':
            condition.note = list()
            annotation = Annotation.construct()
            annotation.text = row['Note']
            condition.note.append(annotation)
        ...
        condition.subject = {'reference':'Patient/'+patient_response['entry'][0]['resource']['id']}
        print(json.loads(condition.json()))
        response = requests.post(target_ingestion_endpoint+tenant_name+'/'+actualresource,
                json=json.loads(condition.json()),
                auth=(target_ingestion_username , target_ingestion_password)).json()
        print(response)
        print(response['id'])
