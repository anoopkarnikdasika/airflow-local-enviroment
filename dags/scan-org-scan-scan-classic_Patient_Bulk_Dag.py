...
mpowered_tenant_table = Variable.get("mpowered_tenant_table")
mpowered_resource_table = Variable.get("mpowered_resource_table")
mpowered_affliate_table = Variable.get("mpowered_affliate_table")
mpowered_user_table = Variable.get("mpowered_user_table")


organization_name = 'Scan ORG'
affliate_name = 'Scan'
lob_name ='Scan Classic'
organization_id = '7237'
affliate_id = '2739'
lob_id = '3653'
consumer_tenant_name = 'DEFAULT'
tenant_name = '7237_2739'
resources = ['Patient']
actualresource = 'Patient'
file_type = 'csv'
name = 'scan-org-scan-scan-classic'
...
def ingest_to_smilecdr(ds, **kwargs):
    ...
    def ingest(i):
        ...
        identifier1.system = row['identifier_0_system']
        identifier1.value = str(int(row['identifier_0_value']))
        identifier2 = Identifier.construct()
        identifier2.system = row['identifier_1_system']
        identifier2.value = str(int(row['identifier_1_value']))
        patient.identifier.append(identifier1)
        patient.identifier.append(identifier2)
        humanName = HumanName.construct()
        patient.name = list()
        humanName.family = row['name_0_family']
        humanName.given = list()
        humanName.given.append(row['name_0_given_0'])
        humanName.use = row['name_0_use']
        patient.name.append(humanName)
        patient.telecom = list()
        contactPoint1 = ContactPoint.construct()
        contactPoint1.system = row['telecom_0_system']
        contactPoint1.value = row['telecom_0_value']
        patient.telecom.append(contactPoint1)
        contactPoint2 = ContactPoint.construct()
        contactPoint2.system = row['telecom_1_system']
        contactPoint2.value = row['telecom_1_value']
        patient.telecom.append(contactPoint2)
        patient.gender = row['gender']
        date_obj = datetime.strptime(row['birthDate'], "%d-%m-%Y")
        year = str(date_obj.year)
        month = str(date_obj.month)
        day = str(date_obj.day)
        date_string = year + '-' + month + '-' + day
        patient.birthDate = date_string
        patient.address = list()
        address = Address.construct()
        address.line = list()
        address.line.append(row['address_0_line_0'])
        address.city = row['address_0_city']
        address.state = row['address_0_state']
        address.postalCode = str(int(row['address_0_postalcode']))
        address.use = row['address_0_use']
        address.text = row['address_0_text']
        ...
        response = requests.post(target_ingestion_endpoint+consumer_tenant_name+'/'+actualresource,
                json=patient_json,headers=headers,
                auth=(target_ingestion_username , target_ingestion_password)).json()
        print(response['id'])
        # id = response['id']
        #     # print("newly ingested")
        # print(id)
...
task_concatenate >> task_standardization_mapped >> task_ingest_to_smilecdr >> task_update_postgres