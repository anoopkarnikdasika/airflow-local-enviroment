...
def ingest_to_smilecdr(ds, **kwargs):
    ...
    def ingest(i):
        ...
        invoice['identifier']= list()
        invoice['identifier'].append({'system':'data_source','value':'Scan'})
        invoice['identifier'].append({'system':'invoice_id','value':row['Bill Id']})
        if row['Status']!='':
            status = row['Status']
            invoice['status'] = status
        if row['Bill Date']!='':
            invoice['date'] = str(row['Bill Date'])
        if row['total Due Amount']!='':
            invoice['totalNet'] = {'value':str(row['total Due Amount'])}
            invoice['totalGross'] = {'value':str(row['total Due Amount'])}
        invoice['extension']=[]
        if row['Payment status']!='':
            invoice['extension'].append({'url':'paymentStatus','valueString':row['Payment status']})
        if row['Payment Date']!='':
            invoice['extension'].append({'url':'payment_date','valueString':row['Payment Date']})
        
        print(f"organization url = {target_ingestion_endpoint}{tenant_name}/Organization?name={row['Organization Name']}")
        org_response = requests.get(target_ingestion_endpoint+tenant_name+'/Organization?name='+row['Organization Name'],
                auth=(target_ingestion_username , target_ingestion_password)).json()
        print(org_response)
        if org_response['total'] ==0:
            organization = {}
            organization['resourceType'] = "Organization"
            organization['name'] = row['Organization Name']
            org_response = requests.post(target_ingestion_endpoint+tenant_name+'/Organization',
                json=json.loads(json.dumps(organization)),
                auth=(target_ingestion_username , target_ingestion_password)).json()
            print(org_response)
            org_id = org_response['id']
        else:
            org_id = org_response['entry'][0]['resource']['id']
        invoice['issuer'] = {'reference':'Organization/'+org_id}
        print(json.loads(json.dumps(invoice)))
        response = requests.post(target_ingestion_endpoint+tenant_name+'/'+actualresource,
                json=json.loads(json.dumps(invoice)),
                auth=(target_ingestion_username , target_ingestion_password)).json()
        print(response)
        print(response['id'])
