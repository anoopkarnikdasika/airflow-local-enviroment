...
def ingest_to_smilecdr(ds, **kwargs):
    ...
    def ingest(i):
        ...
        observation.identifier = list()
        identifier2 = Identifier.construct()
        identifier2.system = "data_source"
        identifier2.value = "Scan"
        observation.identifier.append(identifier2)
        observation.category = list()
        category = CodeableConcept.construct()
        category.coding = list()
        category.coding.append({'display':'vital-signs','code':'vital-signs'})
        category.text = 'vital-signs'
        observation.category.append(category)
        if row['Code']!='':
            code = CodeableConcept.construct()
            code.coding = list()
            code.coding.append({'display':row['Name'],'code':row['Code']})
            code.text = row['Name']
            observation.code = code
        if row['Recorded_on']!='':
            observation.effectiveDateTime = row['Recorded_on']
        if row['Recorded_by']!='':
            observation.performer = list()
            observation.performer.append({'display':row['Recorded_by']})
        if row['Value']!='':
            print(row['Value'])
            if ' ' in row['Value']:
                value = row['Value'].split(' ')[0]
                unit = row['Value'].split(' ')[1]
                observation.valueQuantity = Quantity.construct()
                observation.valueQuantity.value = value
                observation.valueQuantity.unit = unit
            else:
                observation.valueQuantity = Quantity.construct()
                observation.valueQuantity.value = row['Value']
        if row['Reference_range']!='':
            observation.referenceRange = list()
            try:
                referenceRanges = [x.strip() for x in row['Reference_range'].split('-')]
                low_value = referenceRanges[0]
                high_value = referenceRanges[1]
                if 'F' in low_value:
                    low_unit = low_value[-2:]
                    low_value = low_value[:-3]
                    low = {'value':str(low_value),'unit':low_unit}
                elif 'cm' in low_value:
                    low_unit = low_value[-2:]
                    low_value = low_value[:-2]
                    low = {'value':str(low_value),'unit':low_unit}
                elif 'KG' in low_value:
                    low_unit = low_value[-2:]
                    low_value = low_value[:-3]
                    low = {'value':str(low_value),'unit':low_unit}
                else:
                    low = {'value':str(low_value)}
            
                if 'F' in high_value:
                    high_unit = high_value[-2:]
                    high_value = high_value[:-3]
                    high = {'value':str(high_value),'unit':high_unit}
                elif 'cm' in high_value:
                    high_unit = high_value[-2:]
                    high_value = high_value[:-2]
                    high = {'value':str(high_value),'unit':high_unit}
                elif 'KG' in high_value:
                    high_unit = high_value[-2:]
                    high_value = high_value[:-3]
                    high = {'value':str(high_value),'unit':high_unit}
                else:
                    high = {'value':str(high_value)}

                observation.referenceRange.append({'low':low,'high':high,'text':row['Reference_range']})
            except:
                print(f"{row['Reference_range']} is not defined correctly")
        if row['Test_Result']!='':
            observation.interpretation = list()
            observation.interpretation.append({'coding':[{'code':row['Test_Result']}]})
        ...
        observation.subject = {'reference':'Patient/'+patient_response['entry'][0]['resource']['id']}
        print(json.loads(observation.json()))
        response = requests.post(target_ingestion_endpoint+tenant_name+'/Observation',
                json=json.loads(observation.json()),
                auth=(target_ingestion_username , target_ingestion_password)).json()
        print(response)
        print(response['id'])
